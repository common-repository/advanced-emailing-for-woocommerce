...
use rnadvanceemailingwc\Utilities\ObjectSanitizer;
...
abstract class AjaxBase
{
    ...
    public function GetRequiredRegex($propertyName,$pattern,$errorMessage='')
    {
        $value=$this->GetRequired($propertyName);
        if(!preg_match($pattern,$value))
            $this->SendErrorMessage($errorMessage==''?'Invalid property '.$propertyName:$errorMessage);
        return $value;
    }
    ...
    public function GetRequiredNumber($propertyName)
    {
        return intval($this->GetRequired($propertyName));
    }
